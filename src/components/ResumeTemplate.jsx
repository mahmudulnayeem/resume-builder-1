...
import { Linkedin, Github, Facebook, Twitter, Instagram } from './../images/icon/socialIcon'
        ...
        "experiences": [
            {
                ...
                "from": "2023-05-10",
                "to": "2023-05-10",
                "description": "Led scope, planning, and design of the Checkout API with 3 teams and over 25 stakeholders., The Checkout API reduced integration issues by 18% over its 100,000+ users."
            },
            {
                "title": "Web Developer",
                "companyName": "Github",
        ...
        <div className='max-w-[1000px] mx-auto py-10  border-2 border-x-white'>
            ...
            <header className='mx-9 px-16 grid grid-cols-3 py-8 items-center'>
                <div className='col-span-1'>
                    <h1 className='text-4xl font-semibold mb-2'>{data?.personalDetails?.name}</h1>
                    <p className='font-semibold mb-1'>{data?.personalDetails?.profession}</p>
                    <p>{data?.personalDetails?.address?.city}, {data?.personalDetails?.address?.country}</p>

                    <p className='my-1'>{data?.personalDetails?.email}</p>
                    <p>{data?.personalDetails?.phone}</p>
                </div>
                <div className='col-span-2'>
                    <p >{data?.personalDetails?.motivation}
                    </p>
                    <div className='flex gap-2 my-2 justify-end'>
                        {data?.socials?.linkedin && <a className="rounded-lg" href={data?.socials?.linkedin} target='_blank'><Linkedin  className="text-blue-600"/></a>}
                        {data?.socials?.github && <a className="rounded-lg" href={data?.socials?.github} target='_blank' ><Github /></a>}
                        {data?.socials?.twitter && <a className="rounded-lg" href={data?.socials?.twitter} target='_blank' ><Twitter className="text-blue-600"/></a>}
                        {data?.socials?.instagram && <a className="rounded-lg" href={data?.socials?.instagram} target='_blank' ><Instagram className="text-red-600"/></a>}
                        {data?.socials?.facebook && <a className="rounded-lg" href={data?.socials?.facebook} target='_blank' ><Facebook className="text-blue-500"/></a>}
                    </div>
                </div>
            </header>
                        ...
                        data?.experiences?.map(WE =>
                            ...
                            <div>
                                <div className='flex justify-between items-center mt-4 mb-3'>
                                    ...
                                    <p className='font-semibold text-gray-600'>{WE?.from} - {WE?.to}</p>
                                </div>
                    ...
                    <div className='mb-4'>
                        {data.educations?.map(ED =>
                            <div className='flex justify-between items-center mt-3 mb-3'>
                                ...
                                <p className='font-semibold text-gray-600'>{ED?.from} - {ED?.to}</p>
                            </div>
